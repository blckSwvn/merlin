...
use nodes::{Nodes, NodeIdx, LeafIdx, Split, SplitIdx, Direction};
use screen::{ScreenBuffer, Cell};
...
impl From <ropey::Error> for EditorErr {
...
impl Anchor {
    ...
    fn get_enumerated(&self, curr_dimension: u16, parent_dimension: u16) -> u16 {
        match self {
            ...
            Anchor::Negative(n) => u16::max(curr_dimension.saturating_sub(*n), parent_dimension),
...
use cmdline::{CmdLine, CmdLineDummy};
enum Arg{
    FilePath,
    Numeric,
}
...
const COMMANDS: [Cmd; 11] = [
    ...
    Cmd{
        name: "open",
        argument: Some(Arg::FilePath),
    },
    Cmd{
        name: "close",
        argument: Some(Arg::Numeric),
    },
    Cmd{
        name: "buffer-switch",
        argument: Some(Arg::Numeric),
    ...
    impl Component for CmdLineDummy {
        ...
        fn behaviour(
            ...
            // fn parse_cmd(cmd_line: &mut CmdLine)->Result(Cmd, String>{
            ...
            fn exec_cmd(
                ...
                match cmd {
                    ...
                    Cmd::Exec => {},
                    // Cmd::Exec => match parse_cmd(cmd_line){
                    //     Ok(c)=> exec_cmd(c, cmd_line, nodes, focus, views, buffers)?,
                    //     // Err(e)=>return Err(EditorErr::Msg(e)),
                    // },
                    Cmd::EnterNormal => {
...
mod screen {
    use super::{Color, stdout, io, BG, FG};
use crossterm::{
    cursor::{MoveTo},
    queue,
